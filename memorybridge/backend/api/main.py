...
Lifespan:
  - Loads sentence-transformers model into app_state
...
from api.state import app_state  # noqa: E402 — must import after logging setup
...
async def lifespan(app: FastAPI):
    ...
    # Load sentence-transformers model in thread pool (CPU-bound)
    ...
    app_state["model"] = await loop.run_in_executor(None, _load_model)
...
async def proxy_embed(request: Request):
    ...
    This lets the Chrome extension use a single public URL for everything.
    """
    body = await request.json()
    async with httpx.AsyncClient(timeout=60.0) as client:
        resp = await client.post("http://localhost:8765/embed", json=body)
...
async def proxy_store(request: Request):
    """
    Proxy POST /store to Phase 1 Flask server on localhost:8765.
    """
    body = await request.json()
    async with httpx.AsyncClient(timeout=120.0) as client:
        resp = await client.post("http://localhost:8765/store", json=body)
        resp.raise_for_status()
        return resp.json()
